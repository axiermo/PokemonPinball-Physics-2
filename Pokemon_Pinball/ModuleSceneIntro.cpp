...
bool ModuleSceneIntro::Start()
{
	...
	overlay = new element(map, 24, 32, 500, 500, -4, 10);
...
update_status ModuleSceneIntro::Update(){
	...
	if ((App->input->GetKey(SDL_SCANCODE_SPACE) == KEY_DOWN) || (App->input->GetKey(SDL_SCANCODE_SPACE) == KEY_REPEAT))
		ball_launcher_joint->SetMotorSpeed(0.5);
	else
		ball_launcher_joint->SetMotorSpeed(-50);
...
void ModuleSceneIntro::DrawChainsBoard()
{
	...
	ball.physbody = App->physics->CreateCircle(ball_point.x, ball_point.y, 11, b2BodyType::b2_dynamicBody);
	...
	//walls
	Rigthentrance = App->physics->CreateRectangleSensor(370, 420, 30, 0);
	// Pivot 0, 0
	// Pivot 0, 0
	// Pivot 0, 0
	int bumper_rigth[8] = {
	...
	PhysBody* bumperright = App->physics->CreateChain(0, 0, bumper_rigth, 8, b2BodyType::b2_staticBody);
	...
	PhysBody* TunelExterior = App->physics->CreateChain(0, 0, tunel_exterior, 118, b2BodyType::b2_staticBody);
...
//
//	bodyA->body->linearVelocity(v2vec2(-3, -2));
//}
